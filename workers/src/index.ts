...
app.get("/", async (c) => {
  ...
  const r2Objects = (await c.env.r2_buckets.list()).objects.map((obj) => obj.key);
  ...
  return c.json(changelogArr, { headers: { "Content-Type": "application/json" } });
});
...
app.post("/", async (c) => {
  ...
  const date = changelogDate
    ? formatDate(parse(changelogDate, "do MMM, yyyy H:m", new Date()), "do MMM, yyyy H:m")
